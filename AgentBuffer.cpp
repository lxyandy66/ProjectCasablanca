...
double AgentBuffer::getData() {
    return data;
}
void AgentBuffer::setData(double dt) {
    this->data = dt;
}
...
AgentBuffer AgentBuffer::msgToAgentBuffer(AgentMsg msg) {
    ...
    return AgentBuffer(msg.boardId, msg.boardType, msg.reqId,
                       jsonTemp[AgentProtocol::DATA_TEMP_FROM_JSON].as<double>());
}
...
int AgentBufferList::listSize() {
    return this->list.size();
}

void AgentBufferList::add(AgentBuffer ab) {
    // Serial.println("AgentBuffer will push: detail:" + ab.getBoardId() + ab.getBoardType() + ab.getReqId() +
    // ab.getData());
...
void AgentBufferList::setListType(String str) {
    this->listType = str;
}
